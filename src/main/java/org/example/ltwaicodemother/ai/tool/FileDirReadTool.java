...
public class FileDirReadTool extends BaseTool {
    ...
    public String readDir(@P("目录的相对路径，为空则读取整个项目结构")
                          String relativePath,
                          @ToolMemoryId Long appId){
        try{
            Path path = Path.of(relativePath==null?"":relativePath);
            if(!path.isAbsolute()){
                String projectDirName="vue_project"+appId;
                Path projectRoot = Path.of(AppConstant.CODE_OUTPUT_ROOT_DIR, projectDirName);
                path = projectRoot.resolve(relativePath==null?"":relativePath);
            }
            File targetDir=path.toFile();
            if(!targetDir.exists() || !targetDir.isDirectory()){
                return "错误：目录不存在或者不是目录 -"+relativePath;
            }
            StringBuilder structure=new StringBuilder();
            structure.append("项目结构目录：\n");
            ...
            allFiles.stream().sorted((f1,f2)->{
                ...
                if(f1!=f2) return Integer.compare(depth1,depth2);
